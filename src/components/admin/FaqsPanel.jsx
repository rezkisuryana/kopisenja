        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-charcoal-50">
              {faqs.map((f, i) => (
                <tr key={f.id} className="hover:bg-charcoal-50/50 transition-colors">
                  <td className="px-6 py-4 text-sm text-charcoal-400 font-medium">{i + 1}</td>
                  <td className="px-6 py-4 text-sm font-semibold text-charcoal-800 max-w-xs">{f.question}</td>
                  <td className="px-6 py-4 text-sm text-charcoal-500 max-w-sm line-clamp-2">{f.answer}</td>
              ...
              {!faqs.length && (
                <tr><td colSpan={4} className="px-6 py-10 text-center text-charcoal-400 text-sm">Belum ada FAQ.</td></tr>
              )}
            </tbody>
          </table>
        </div>
